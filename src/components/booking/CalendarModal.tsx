  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center z-50">
      ...
      <div className="relative bg-white rounded-lg shadow-lg w-full max-w-2xl sm:max-w-lg">
        <div className={`w-full border border-gray-300 rounded-lg p-2 ${darkMode ? 'bg-gray-900 text-white' : 'bg-white'}`}>
          <Calendar
            className="w-full text-xl"
            onChange={handleDateChange}
            value={selectedDate}
            tileClassName={({ date, view }) => {
              const baseClasses = "flex justify-center items-center w-full h-12 text-lg font-semibold";
              const nowClasses = date.toDateString() === new Date().toDateString() ? "bg-green-500 text-white border border-green-500" : "";
              const activeClasses = view === 'month' && date.toDateString() === date.toDateString() ? "bg-green-500 text-white font-bold" : "";
