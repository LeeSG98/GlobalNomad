      ...
      >
        {links.map((link, index) => (
          <Link key={link.id} href={`/experiencedetail/${link.id}`} passHref>
            <div
              className={`relative box-border h-[384px] w-[384px] flex-shrink-0 cursor-pointer overflow-hidden rounded-2xl scroll-snap-align-start ${
                index === 0 ? "ml-0" : "mx-3"
              }`}
            >
              <div
                className="absolute inset-0 transform bg-cover bg-center transition-transform duration-300 ease-in-out"
                style={{
